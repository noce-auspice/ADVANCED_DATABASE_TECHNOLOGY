# Agricultural Distributed Database Management System - README

Project Overview

This project implements a distributed agricultural database management system using PostgreSQL with horizontal fragmentation, distributed queries, and business rule enforcement. The system manages farm data across multiple database nodes with comprehensive data integrity and performance optimization.

Case Study: Agricultural Harvest Management

Business Context
- Farm Operations: Tracks agricultural fields, crop types, and harvest yields
- Distributed Architecture: Data distributed across Node_A and Node_B
- Data Integrity: Ensures consistency across distributed fragments
- Business Rules: Enforces agricultural operational constraints

System Architecture

Database Nodes
- Node_A: Primary node with main operations and local fragment (Harvest_A)
- Node_B: Secondary node with complementary fragment (Harvest_B)

Core Tables
- `Field`: Agricultural field information (location, size)
- `Crop`: Crop types and seasonal data
- `Harvest`: Main fact table for harvest records (fragmented horizontally)
- `Harvest_A`: Local fragment (harvest_id 1-5)
- `Harvest_B`: Remote fragment (harvest_id 6-10)

Key Features Implemented

A1: Horizontal Fragmentation & Recombination
- Range-based fragmentation on `harvest_id` (1-5 → Node_A, 6-10 → Node_B)
- Unified view (`Harvest_ALLLL`) combining both fragments using `dblink`
- Data validation with checksums and row counts

A2: Cross-Node Distributed Joins
- Database links between Node_A and Node_B
- Distributed queries joining local harvest data with remote crop information
- Selective predicates to maintain 3-10 row result sets

A3: Parallel vs Serial Aggregation
- Performance comparison between serial and parallel execution plans
- Query optimization with parallel worker configuration
- Execution statistics analysis for aggregation operations

A4: Two-Phase Commit & Recovery
- Distributed transactions across Node_A and Node_B
- Transaction recovery mechanisms for system failures
- Consistency verification after commit operations

A5: Distributed Lock Management
- Lock conflict simulation across multiple sessions
- Real-time lock diagnostics and monitoring
- Deadlock prevention strategies

B6: Declarative Rules Hardening
- Data integrity constraints (NOT NULL, CHECK constraints)
- Domain validation for crop seasons and yield values
- Comprehensive error handling for constraint violations

B7: E-C-A Trigger for Denormalized Totals
- Audit trail implementation for crop yield changes
- Automatic total calculation across distributed fragments
- Trigger-based consistency maintenance

B8: Recursive Hierarchy Roll-Up
- Multi-level hierarchy (Farm → Fields → Crops → Harvests)
- Recursive CTEs for hierarchical data traversal
- Roll-up aggregations at different hierarchy levels

B9: Mini-Knowledge Base with Transitive Inference
- Semantic triple store for agricultural knowledge
- Recursive inference of crop types and properties
- Property inheritance through type hierarchies

B10: Business Limit Alert System
- Configurable business rules for yield limits
- Automatic violation detection via triggers
- Graceful error handling with proper rollbacks
